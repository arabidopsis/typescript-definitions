...
use super::{ident_from_str, QuoteT};
...
impl StructBuilder {
    fn new<'a>(fields: &[ast::Field<'a>], ct: &attr::Container) -> StructBuilder {
        let content = fields.into_iter().map(|field| Pair::new(&field));
        // let name = ct.name().serialize_name();
        StructBuilder {
            attrs: content.collect::<Vec<_>>(),
        }
    }
}
...
impl EnumBuilder {
    fn new<'a>(variants: &[ast::Variant<'a>], attrs: &attr::Container) -> EnumBuilder {
        ...
        let content = variants.into_iter().map(|variant| {
            ...
            match variant.style {
                ast::Style::Struct => {
                    let b = Box::new(StructVariantBuilder::new(
                        &taginfo,
                        &variant_name,
                        &variant.fields,
                    ));
                ...
                ast::Style::Newtype | ast::Style::Tuple | ast::Style::Unit => {
                    let b = Box::new(TupleVariantBuilder::new(
                        &taginfo,
                        &variant_name,
                        &variant.fields,
                    ));
