...
pub fn derive_enum<'a>(
   ...
   let taginfo = match attrs.tag() {
        ...
        EnumTag::Adjacent {tag, content, ..} => TagInfo {tag, content: Some(&content)},
        _ => TagInfo { tag: "kind", content: None}
    };
    variants
        .into_iter()
        .map(|variant| {
            ...
            match variant.style {
                ...
                ast::Style::Tuple => {
                    derive_tuple_variant(&taginfo, &variant_name, &variant.fields)
                }
